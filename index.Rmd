...
# Overview
This document outlines the process and materials required to create the happiness dashboard, within the app.R file. 
...
Another issue is that the data contains manyy NA values for small, or less developed countries. This means the projects conclusions, especially that of the scatter plots, may be biased away from less economically developed countries. The World Bank’s solution to this was to impute the NAs with entries from the most recent year of record. This project decided to avoid this as to not mislead the reader with outdated information. However, the project could be updated to impute past dates, and then record their entry date in the data explorer.

Finally, population density was massively skewed by highly dense city states (Hong Kong and Singapore), so a log transformation was decided to reduce their influence. Log transformation reduces the values of extreme scores. This was also conducted to the colour palette of the leaflet map, as to allow for a greater range of polygon colour, and thus better interpretation of true differences in population density. However, log transformations make it harder to convey exact relative differences in population density.

The extent of population density's skew can be seen below. It thus makes it very hard to interpret a meaningful relationship between a country's population density and their happiness. This is especially the case when city states could be argued as not being true countries. It could be argued removing these data points is a better solution, however their inclusion still offers valuable information to all the other relationships in the dashboard and to population density also, once their influence was reduced. 

![](www/pop_skew.png)

Log transforming the map's colour palette appears to create an intuitively realistic and informative distribution of polygon colour, which is the major goal of the visualisation. As can be seen below, when population density was unlogged the extreme skew caused the values of all other countries to be interpreted as relatively small, and thus their polygons were filled pale purple accordingly. This would make the informational value of this graph redundant. 

![](www/pop_unlogged.png)


GDP per capita's colour palette was also log transformed. However, its scatterplot was not. The colour palette was log transformed as the extremely rich countries were skewing the colour palette, whilst creating the unusual situation where leaflet was not utilising the full range of the darker colour palette. This was caused by Luxembourg skewing the palette. This left the majority of countries coloured in pale yellow, making it hard to discern true differences in countries' wealth. For instance, Argentina with a GDP per capita of \$8,579 was coloured the same as the Central African Republic with a GDP per capita of \$493. This is very misleading and reduces the informational wealth of the visualisation. The colour palette of an unlogged GDP per capita value can be seen below.

![](www/gdp_unlogged.png)


However, the GDP per capita scatter plot was left unlogged. This is because, even with Luxembourg's extreme wealth, it did not overly skew the distribution of data points. This created an informative graph to highlight the relationship between variations in a country's GDP per capita and wealth, as can be seen below.

![](www/gdp_scatter.png)

Logging the values, would make the relationship hard to quantify and unnatural, so it should only be applied to graphs in extreme cases, such as population density. Although logging the polygon palette removes the linearity of comparison between colours, it could be argued this is less impactful on a choropleth map. This is because a choropleth graph aims to inform the reader on which countries score comparatively higher or lower, along with immediately signalling the highest and lowest scoring countries. Therefore, by logging the palette, it creates a wider range of colour values to signify differences in scores. 
...
**mytext** uses the *sprintf* function, which passes C style text formatting, and replaces each '%s' in the string with a variable. Which variable is inserted into the string, depends on the order they are listed. The text formatting bolds the top line of text, to highlight the key information for the country's name. It was decided not to include the units of measurement for population density (per km$^2$), as this over complicated the label, and units of measurement were signaled in population density's legend.
...
#custom colour palette
mypalettegdp <- colorBin( palette="YlGn", domain=worldCountries@data$gdp_log, bins = 20)
